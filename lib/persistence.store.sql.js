  ...
  persistence.schemaSync = function (tx, callback, emulate) {
    ...
    for (var entityName in entityMeta) {
      if (entityMeta.hasOwnProperty(entityName)) {
        ...
        for (var i=0; i<meta.index.length; i++) {
          var cols;
          if(typeof meta.index[i] == 'object') { // lets suppose we've ogt an array here
            cols = meta.index[i];
          } else if (typeof meta.index[i] == 'string') {
            cols = [meta.index[i]]; // got single col, need to pass an array to createIndex
          }
          queries.push([dialect.createIndex(meta.name, cols), null]);
        }
        if (!meta.isMixin) {
